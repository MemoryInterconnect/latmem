...
//  long ways = 16;
  ...
  //-- run input sie one time
  for (test_size = test_range; test_size <= test_range; test_size <<= 1) {
    long i, j, n, delta;
    long sum, sum2;
    ...
    //------------ ----------------------------------------------------------
    //-- changed to infinite loop by kyw @2025.0115 to run latmemfar infinitely for adding load to system processor
    //-- from here ----------------------------------------------------------
    do {
        sum = 0;
        sum2 = 0;
        ...
        //printf("%10ld %8.3lf %10.3lf %10ld\n", test_size, mean, sqrt(var), n);  
        //printf("%10ld kB\t%7.3lf %10.3lf %10ld\n", test_size/1024, mean, sqrt(var), n);     //--- messages suppressed
    } while (1);
    //-- to here ------------------------------------------------------------
  }
